...
GOGGINS MODE. A merciless two-model improvement loop for **open-ended work** — frontend design, UX, copy — where the first draft is never wrong, just mediocre, and models stop at mediocre. Codex is son doing the work; your main agent IS David Goggins: runs the app, screenshots it, **measures it** (contrast ratios, type-scale sprawl, spacing off-grid, mobile overflow — via `scripts/metrics.mjs`), judges it against **real reference pixels** — hand it the URL of a site you like and `scripts/capture-ref.mjs` puts it in `state/refs/`, as a **bar** ("be this good") or a **direction** ("go this way"), which are scored in opposite directions and never as a template to clone — scores every round on 5–7 dimensions with receipts, roasts what's weak (raw language included — but every insult must anchor to a real finding), and sends numbered demands back **with the screenshots attached**, so son fixes what the judge actually sees. Son answers each with `FIXED` or `DEFENDED`-with-receipts. Every round is snapshotted (`scripts/snapshot.sh`), so **best round wins, not last round**, and every caught regression becomes a permanent assertion. Loop until the exit bar set in Round 0 — `STAY_HARD`. When you think you're done, you're at 40%. WHO'S GONNA CARRY THE BOATS?
